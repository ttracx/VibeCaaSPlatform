 ...
 */
export async function createGSAPFallback(config) {
 ...
 */
export async function createGSAPTimeline(config) {
    if (typeof window === 'undefined')
        return null;
    try {
        const { gsap } = await import('gsap');
 ...
 */
export async function applyGSAPAnimation(elements, animation, config) {
    if (typeof window === 'undefined')
        return null;
    try {
        const { gsap } = await import('gsap');
