 ...
 */
export function useCanvasSequence(config: CanvasSequenceConfig) {
  ...
  const animate = useCallback((timestamp: number) => {
    if (!isPlaying || !isLoaded || isReducedMotion) return;
  ...
  const handleScroll = useCallback((progress: number) => {
    if (isReducedMotion || !isLoaded) return;
